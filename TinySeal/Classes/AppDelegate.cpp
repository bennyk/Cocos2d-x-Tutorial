...
bool AppDelegate::applicationDidFinishLaunching() {
    ...
    if(!glview) {
        ...
        glview = GLViewImpl::createWithRect("TinySeal", Rect(0, 0, designResolutionSize.width, designResolutionSize.height));
#else
        glview = GLViewImpl::create("TinySeal");
#endif
    ...
    CCLOG("setting content scale factor %.2f (rounded %.2f)", scaleFactor, roundf(scaleFactor));
    director->setContentScaleFactor(roundf(scaleFactor));
